...
Experiment Configuration (YAML files in evocoder/experiments/)
...
evocoder/core/EvolutionaryController.py (EvolutionaryController class)
...
evocoder/problems/<problem_name>/ (Problem Definition Packages)
...
evocoder/llm_interface/ (LLM Interaction Layer)
...
evocoder/core/evaluator_cascade.py (Evaluator class)
...
evocoder/core/program_database.py (ProgramDatabase class)
...
EvolutionaryController fetches a diverse pool of candidate programs (best, random correct, recent) from ProgramDatabase.

From this pool, a parent program is selected using tournament selection (correctness first, then primary metric). Inspiration programs are also selected.

The parent code, inspiration programs, and problem-specific instructions (asking for a diff) are passed to LLMManager.

LLMManager uses the configured LLM provider (e.g., OpenWebUIProvider) to send a prompt to the LLM.

The LLM returns a diff string.

EvolutionaryController uses diff_utils.apply_diffs to apply the diff to the parent code, creating the evolved code string.

The evolved code string is passed to the Evaluator.

Evaluator runs the evaluation cascade (e.g., pytest stages with markers) and returns a dictionary of scores.

EvolutionaryController stores the evolved code, its scores, the original diff string, and lineage information into the ProgramDatabase.

The loop repeats for the configured number of generations. All significant events and errors are logged via the logging system.
